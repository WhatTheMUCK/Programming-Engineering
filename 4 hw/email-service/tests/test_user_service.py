...
def generate_unique_email():
    ...
    return f"test_{uuid.uuid4().hex[:16]}@example.com"
...
async def test_create_user_success(service_client):
    ...
    print(f"[TEST] Creating user with login={user_data['login']}, email={user_data['email']}", file=sys.stderr)
    response = await service_client.post('/api/v1/users', json=user_data)
    print(f"[TEST] Response status: {response.status}, text: {response.text}", file=sys.stderr)
